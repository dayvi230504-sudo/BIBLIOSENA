...
def crear_prestamo():
    ...
    try:
        ...
        if (elemento.cantidad_disponible or 0) <= 0:
            ...
            w = WaitlistDB(id=str(uuid.uuid4()), id_elemento=id_elemento, id_usuario=id_usuario_final, contacto=contacto, estado='pendiente', creado_en=noww, actualizado_en=noww)
            db.add(w)
            db.commit()
...
def devolver_prestamo(prestamo_id: str):
    ...
    try:
        ...
        if not elemento:
            return jsonify({"ok": False, "error": "Elemento no encontrado"}), 404
        elemento.cantidad_disponible = int((elemento.cantidad_disponible or 0) + 1)
        elemento.estado_disponibilidad = 'Disponible'
        elemento.actualizado_en = datetime.utcnow()
        p.estado = 'devuelto'
...
def conectar_usuarios():
    ...
    try:
        now = datetime.utcnow()
        # Crear mensaje de conexión en ambas direcciones
        msg1 = MensajeDB(
            id=str(uuid.uuid4()),
            id_remitente='admin',
            id_destinatario=id_usuario1,
            asunto='Conexión iniciada',
            contenido=f"Has sido conectado con otro usuario. {mensaje_inicial}",
            leido=0,
            tipo='chat',
        ...
        db.add(msg2)
        db.commit()
        return jsonify({"ok": True, "mensaje": "Usuarios conectados. Pueden chatear entre sí ahora."}), 201
    finally:
        db.close()
